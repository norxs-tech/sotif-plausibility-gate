 ...
 *
 *              Degradation Matrix (most severe matching condition wins):
 *                - Any E2E counter error              → kEmergencyStop
 *                - Any ASIL-D internal invariant fail → kEmergencyStop
 *                - LiDAR + Radar both failed          → kMinimalRiskCondition
 *                - LiDAR failed                       → kRadarCameraFallback
 *                - Radar failed                       → kLidarCameraFallback
 *                - Camera failed                      → kLidarRadarFallback
 *                - GNSS failed                        → kDeadReckoningMode
 *                - IMU failed                         → kReducedDynamics
 *                - All sensors nominal                → kFullOperation
 *
 *              ComputeRequiredState() evaluates every condition and escalates
 *              to the MOST severe matching SafeState (severity = the
 *              SafeState enum's own declared numeric order), not the first
 *              matching condition — compound sensor loss must never resolve
 *              to a less restrictive state than a subset of the same faults
 *              would. (Fixed 2026 — see CHANGELOG.md; the prior
 *              priority-ordered if/return chain under-escalated 43 of the 32
 *              possible fault-domain combinations' supersets, found by
 *              sotif-plausibility-gate/formal-verification's exhaustive
 *              state-space proof, Finding F-02.)
 *
...
//
// Implements the Degradation Matrix table from the header comment: evaluates
// every condition and escalates to the MOST severe matching SafeState,
// rather than returning on the first match — see the file-header comment
// for why (Finding F-02).
// ============================================================================
...
SafeState SafetyArbitrator::ComputeRequiredState() const noexcept {
    ...
    // Mandatory domain(s) lost beyond threshold → MRC. This is an
    // independent, unconditional latch on a different signal (accumulated
    // faultCount over kFaultThreshold ticks, not just the current bitmask)
    // and is outside the scope of Finding F-02's exhaustive bitmask proof;
    // left unchanged by this fix. Safe to return immediately: MRC is
    // already the maximum severity this function can produce (kEmergencyStop
    // is set elsewhere, never by this function), so no downstream escalation
    // could ever raise it further.
    ...
    // Degradation Matrix: escalate to the most severe matching condition.
    // SafeState's own declared enum order IS the severity order (see
    // SafetyArbitrator.hpp), so a plain integer comparison is the severity
    // comparison — not a separately-invented ranking that could disagree
    // with the real enum.
    ...
    if (lidarFailed && radarFailed)  { Escalate(SafeState::kMinimalRiskCondition); }
    if (lidarFailed && !radarFailed) { Escalate(SafeState::kRadarCameraFallback); }
    if (radarFailed && !lidarFailed) { Escalate(SafeState::kLidarCameraFallback); }
    if (cameraFailed)                { Escalate(SafeState::kLidarRadarFallback); }
    if (gnssFailed && !imuFailed)    { Escalate(SafeState::kDeadReckoningMode); }
    if (imuFailed)                   { Escalate(SafeState::kReducedDynamics); }
